...
def init_rcs_3():
    ...
    for m in range(0, 7, 3):
        for n in range(0, 7, 3):
            ...
            S[SQUARE_MAP[(m, n)]] = (full_set - s_set, [])
...
def possible_values_for_cell_3(i, j):
    return R[i][0].difference(R[i][1]).intersection(C[j][0].difference(C[j][1]), S[SQUARE_MAP[(i, j)]][0].difference(S[SQUARE_MAP[(i, j)]][1]))


def update_rcs_3(i, j, val, ):
    global R, C, S

    R[i][1].append(val)
    C[j][1].append(val)
    S[SQUARE_MAP[(i, j)]][1].append(val)



def delete_rcs_3(i, j, ):
    global R, C, S

    R[i][1].pop()
    C[j][1].pop()
    S[SQUARE_MAP[(i, j)]][1].pop()

def recursive_sudoku():
    global EMPTY_CELLS

    for i, j, in EMPTY_CELLS:
        possible_values = possible_values_for_cell_3(i, j)

        for val in possible_values:

            update_rcs_3(i, j, val, )
            EMPTY_CELLS.remove((i, j,))
            VALUES_TRACE[(i, j,)] = val
